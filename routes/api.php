    ...
    foreach ($versions as $version) {
        ...
        foreach ($books as $book => $chapters) {
            for ($ch = 1; $ch <= $chapters; $ch++) {
                if (\Illuminate\Support\Facades\Cache::has("bible_chapter_{$version}_{$book}_{$ch}")) {
                    $count++;
                }
            }
        }
        $cached[$version] = $count;
        $totalCached += $count;
    }

    $totalPossible = $totalChapters * count($versions);

    return response()->json([
        'total_chapters_per_version' => $totalChapters,
        'versions' => $cached,
